...
class InstanceUpdateOpResolverTest extends UnitTest with Inside {
  ...
  class Fixture {
    ...
    lazy val reservedInstance = TestInstanceBuilder.newBuilder(appId).addTaskCreated().withReservation().getInstance()
    lazy val existingReservedTask: Task = reservedInstance.appTask

    lazy val reservedLaunchedInstance: Instance = TestInstanceBuilder.newBuilder(appId).addTaskLaunched().withReservation().getInstance()
